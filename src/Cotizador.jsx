...
const SistemaCotizadorOpComplejas = () => {
  ...
  const cAmb=useCallback((a)=>{
    const comb=((a.kms||0)/Math.max(a.rendKmL||3,0.1))*(a.precioComb||0);
    const eecc=Array.isArray(a.eecc)?a.eecc.reduce((s,e)=>s+(e.qty||1)*(e.precioUnit||0),0):0;
    return (a.cm||0)+comb+eecc;
  },[]);
  // Costo total mensual de un trailer: alquiler/amortización + combustible + EECC.
  const cTr=useCallback((t)=>{
    const comb=((t.kms||0)/Math.max(t.rendKmL||3,0.1))*(t.precioComb||0);
    const eecc=Array.isArray(t.eecc)?t.eecc.reduce((s,e)=>s+(e.qty||1)*(e.precioUnit||0),0):0;
  ...
  const renderAmb=()=>{
    ...
    const addD=m=>{const eecc=getCostosEECC('Ambulancias','UTIM');setAmbulancias([...ambulancias,{...m,id:uid(),origen:'disp',impL:ambIG,kms:0,precioComb:precioGasoil,rendKmL:3,eecc,eeccOpen:true}]);rW();};
    ...
    return(<div>
      ...
      <SubBar items={ambulancias} calc={a=>cAmb(a)} color="#dc2626" label="Sub. Ambulancias"/>
      ...
      <div style={{background:'white',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:16}}>
        ...
        {ambWiz.step===2&&ambWiz.orig==='disp'&&<div><button onClick={rW} style={{fontSize:12,color:'#3b82f6',background:'none',border:'none',cursor:'pointer',marginBottom:10}}>← Volver</button>
          {soporteAmb&&soporteAmb.length>0?<div style={{maxHeight:350,overflowY:'auto',border:'1px solid #e2e8f0',borderRadius:8}}><table style={{...sT,fontSize:10}}><thead><tr style={{background:'#dc2626'}}>{Object.keys(soporteAmb[0]).map(k=>(<th key={k} style={{...sTh,color:'white',fontSize:9,padding:4}}>{k}</th>))}</tr></thead><tbody>{soporteAmb.map((r,i)=>{const keys=Object.keys(r);return(<tr key={i} style={{borderBottom:'1px solid #fecaca',cursor:'pointer'}} onClick={()=>{const eecc=getCostosEECC('Ambulancias','UTIM');setAmbulancias([...ambulancias,{id:uid(),mod:r.Modelo||r.Dominio||'Amb '+(i+1),tipo:r.Tipo||'UTI',origen:'disp',tadq:'',cm:0,anio:Number(r['Año']||ANIO_ACTUAL),dom:r.Dominio||'',interno:r.Interno||'',impL:ambIG,kms:0,precioComb:precioGasoil,rendKmL:3,eecc,eeccOpen:true}]);}}>{keys.map((k,j)=>{const v=r[k];const isAnio=/a[nñ]o|year/i.test(k);return(<td key={j} style={{padding:3,fontSize:9}}>{v!=null?(typeof v==='number'&&v>999&&!isAnio?fmt(v):String(v)):''}</td>);})}</tr>);})}</tbody></table></div>:<div style={{padding:24,textAlign:'center',background:'#fef2f2',borderRadius:10,border:'1px dashed #fca5a5'}}><p style={{fontSize:15,fontWeight:700,color:'#dc2626'}}>Sin disponibilidad</p><p style={{fontSize:11,color:'#6b7280',marginTop:6}}>Sin móviles en Back Up. Verificá Dashboard_Moviles_OC en el repo.</p></div>}
        </div>}
      ...
      {ambulancias.map((a,idx)=>{
        const verEecc=Array.isArray(a.eecc);
        const combMensual=((a.kms||0)/Math.max(a.rendKmL||3,0.1))*(a.precioComb||0);
        const eeccTotal=verEecc?a.eecc.reduce((s,e)=>s+(e.qty||1)*(e.precioUnit||0),0):0;
        ...
        return(<div key={a.id} style={{background:'white',borderRadius:8,border:'1px solid #e2e8f0',marginBottom:8}}>
          <div style={{display:'flex',justifyContent:'space-between',padding:12,alignItems:'center'}}>
            ...
            <div style={{display:'flex',gap:10,alignItems:'center'}}>
              {verEecc&&<button onClick={()=>{const n=[...ambulancias];n[idx].eeccOpen=!n[idx].eeccOpen;setAmbulancias(n);}} style={{fontSize:10,padding:'2px 8px',borderRadius:4,border:'1px solid #dc2626',background:a.eeccOpen?'#fef2f2':'white',color:'#dc2626',cursor:'pointer',fontWeight:700}}>{a.eeccOpen?'▼':'▶'} EECC</button>}
          ...
          {verEecc&&a.eeccOpen&&<div style={{padding:12,borderTop:'1px solid #fecaca',background:'#fef9f9'}}>
            {/* Costo base / Amortización / Alquiler */}
            <div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:8,padding:6,background:'white',borderRadius:6}}><span style={{fontWeight:700,color:'#dc2626'}}>{a.origen==='disp'?'Costo mensual base':a.tadq==='Alquiler'?'Alquiler mensual':'Amortización mensual'}</span><input type="number" value={a.cm} onChange={e=>{const n=[...ambulancias];n[idx].cm=Number(e.target.value);setAmbulancias(n);}} style={{width:90,padding:'2px 4px',borderRadius:4,border:'1px solid #e2e8f0',fontSize:11,textAlign:'right'}}/></div>
...
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}><h3 style={{fontSize:16,fontWeight:700,color:'#dc2626'}}>🚗 EECC Móviles</h3><div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{xlMovEeccs.map(ec=>{const cnt=XL_MOVS.filter(m=>m.ec===ec).length;return(<button key={ec} onClick={()=>{setMovEecc(ec);setMovilesXL(XL_MOVS.filter(m=>m.ec===ec).map(m=>({id:uid(),eecc:m.ec,tipo:m.tp,clasif:m.cl,desc:m.de,incluir:m.in,vu:m.vu,qty:m.qt,vm:m.vm,impactos:[]})));}} style={{padding:'4px 10px',borderRadius:6,border:movEecc===ec?'2px solid #dc2626':'1px solid #fca5a5',background:movEecc===ec?'#fef2f2':'white',fontWeight:movEecc===ec?700:400,fontSize:11,cursor:'pointer'}}>{ec} ({cnt})</button>);})}</div></div>
  ...
  const renderTr=()=>{
    ...
    const addD=t=>{const eecc=getCostosEECC('Trailers',null);setTrailers([...trailers,{...t,id:uid(),origen:'disp',impL:trIG,kms:0,precioComb:precioGasoil,rendKmL:3,eecc,eeccOpen:true}]);rW();};
    const addA=(mod,tadq,tipo)=>{
      ...
      setTrailers([...trailers,{
        ...
        kms:0,precioComb:precioGasoil,rendKmL:3,eecc,eeccOpen:true,
      }]);
      rW();
    };
    return(<div>
      ...
      <div style={{background:'white',border:'1px solid #e2e8f0',borderRadius:12,padding:20,marginBottom:16}}>
        ...
        {trWiz.step===3&&trWiz.orig==='disp'&&<div><button onClick={()=>setTrWiz({...trWiz,step:2})} style={{fontSize:12,color:'#d97706',background:'none',border:'none',cursor:'pointer',marginBottom:10}}>← Volver</button>
          {soporteTrail&&soporteTrail.length>0?<div style={{maxHeight:350,overflowY:'auto',border:'1px solid #e2e8f0',borderRadius:8}}><table style={{...sT,fontSize:10}}><thead><tr style={{background:'#d97706'}}>{Object.keys(soporteTrail[0]).map(k=>(<th key={k} style={{...sTh,color:'white',fontSize:9,padding:4}}>{k}</th>))}</tr></thead><tbody>{soporteTrail.map((r,i)=>{const keys=Object.keys(r);return(<tr key={i} style={{borderBottom:'1px solid #fde68a',cursor:'pointer'}} onClick={()=>{const eecc=getCostosEECC('Trailers',null);setTrailers([...trailers,{id:uid(),mod:r.Modelo||r.Dominio||'Trailer '+(i+1),tipo:r.Tipo||'',cat:'Disponible',origen:'disp',neg:negocio,cm:Number(r.CostoMensual||0),impL:trIG,kms:0,precioComb:precioGasoil,rendKmL:3,eecc,eeccOpen:true}]);}}>{keys.map((k,j)=>{const v=r[k];const isAnio=/a[nñ]o|year/i.test(k);return(<td key={j} style={{padding:3,fontSize:9}}>{v!=null?(typeof v==='number'&&v>999&&!isAnio?fmt(v):String(v)):''}</td>);})}</tr>);})}</tbody></table></div>:<div style={{padding:24,textAlign:'center',background:'#fffbeb',borderRadius:10,border:'1px dashed #fbbf24'}}><p style={{fontSize:15,fontWeight:700,color:'#d97706'}}>Sin disponibilidad</p><p style={{fontSize:11,color:'#6b7280',marginTop:6}}>Sin trailers disponibles. Verificá Dashboard_Moviles_OC en el repo.</p></div>}
        </div>}
      ...
      {trailers.map((t,idx)=>{
        const verEecc=Array.isArray(t.eecc);
        const combMensual=((t.kms||0)/Math.max(t.rendKmL||3,0.1))*(t.precioComb||0);
        const eeccTotal=verEecc?t.eecc.reduce((s,e)=>s+(e.qty||1)*(e.precioUnit||0),0):0;
        ...
        return(<div key={t.id} style={{background:'white',borderRadius:6,border:'1px solid #e2e8f0',marginBottom:4}}>
          <div style={{display:'flex',justifyContent:'space-between',padding:8,fontSize:12,alignItems:'center'}}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center'}}>
              {verEecc&&<button onClick={()=>{const n=[...trailers];n[idx].eeccOpen=!n[idx].eeccOpen;setTrailers(n);}} style={{fontSize:10,padding:'2px 8px',borderRadius:4,border:'1px solid #d97706',background:t.eeccOpen?'#fffbeb':'white',color:'#d97706',cursor:'pointer',fontWeight:700}}>{t.eeccOpen?'▼':'▶'} EECC</button>}
          ...
          {verEecc&&t.eeccOpen&&<div style={{padding:12,borderTop:'1px solid #fde68a',background:'#fffdf7'}}>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:8,padding:6,background:'white',borderRadius:6}}><span style={{fontWeight:700,color:'#d97706'}}>{t.origen==='disp'?'Costo mensual base':t.tadq==='Alquiler'?'Alquiler mensual':'Amortización mensual'}</span><input type="number" value={t.cm} onChange={e=>{const n=[...trailers];n[idx].cm=Number(e.target.value);setTrailers(n);}} style={{width:90,padding:'2px 4px',borderRadius:4,border:'1px solid #e2e8f0',fontSize:11,textAlign:'right'}}/></div>
            <div style={{display:'flex',gap:6,alignItems:'center',fontSize:10,marginBottom:8,padding:6,background:'white',borderRadius:6}}>
